...
const Home: React.FC = () => {
    ...
    const ops = useMemo(() => {
        ...
        return t.map(e => {
            ...
            if (!op) return null
            return (
                <SelectableOperator team={team} key={e.id} op={op} />
            )
        })
    }, [team])
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="flex container justify-center gap-4 mt-auto mb-6">
                ...
                <Button className="rounded-none bg-orange-500 text-accent-foreground hover:bg-orange-500/90" onClick={() => setGenerateLoadout(e => !e)}>Random Loadout: <span className="ml-1">{generateLoadout ? "Enabled" : "Disabled"}</span></Button>
                <Button className="rounded-none bg-orange-500 text-accent-foreground hover:bg-orange-500/90" disabled>Random Attachments: <span className="ml-1">Disabled</span></Button>
            </div>
        </div>
    );
}
